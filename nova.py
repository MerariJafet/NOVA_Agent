...
def main():
    parser = argparse.ArgumentParser(
        description='Control del sistema NOVA',
        usage='python nova.py {start|stop}'
    )
    parser.add_argument(
        'command',
        choices=['start', 'stop'],
    ...
    if args.command == 'start':
        try:
            ...
            if result['ollama_pid']:
                print(f"🤖 PID Ollama: {result['ollama_pid']} ({'gestionado' if result['ollama_managed'] else 'externo'})")
