...
def score_model_for_query(model_name: str, signals: Dict[str, Any]) -> int:
    ...
    if signals.get("mentions_image"):
        if "vision" in caps and model_name == "llava:13b":
            return 1000  # LLaVA es el modelo de visión primario
        elif "vision" in caps and model_name == "moondream:1.8b":
            return 500  # moondream como fallback (deprecated)
        else:
...
def route(message: str, has_image: bool = False) -> Dict[str, Any]:
    ...
    if has_image:
        ...
        return {
            "model": "llava:13b",
            "confidence": 100,
            "reasoning": "Imagen adjunta - usando LLaVA end-to-end",
            "alternatives": []
        }
