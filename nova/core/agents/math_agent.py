...
class MathAgent(BaseAgent):
    ...
    def can_handle(self, query: str, context: Optional[Dict] = None) -> float:
        ...
        has_math_expressions = any(
            re.search(pattern, query) for pattern in self.math_patterns
        )
        ...
        keyword_count = sum(
            1 for keyword in self.math_keywords if keyword in query_lower
        )
    ...
    def _analyze_math_query(self, query: str) -> Dict[str, Any]:
        ...
        if any(
            word in query_lower
            for word in ["derivada", "integral", "límit", "limite", "diferencial"]
        ):
            ...
            word in query_lower
            for word in ["ecuación", "sistema", "incógnita", "resolver"]
        ):
            analysis["math_area"] = "algebra"
            analysis["has_equations"] = True
        elif any(
            word in query_lower
            for word in ["demostración", "teorema", "axioma", "postulado"]
        ):
        ...
        analysis["variables"] = sorted(
            list(variables - {"a", "e", "i", "o", "u", "y"})
        )  # Excluir vocales comunes
    ...
    def _extract_math_expressions(self, query: str) -> List[str]:
        ...
        for segment in math_segments:
            ...
            if len(clean_segment) > 2 and any(
                char in clean_segment for char in "+=*/^-()[]{}"
            ):
    ...
    def _provide_explanation(self, analysis: Dict[str, Any]) -> str:
        ...
        return explanations.get(
            math_area,
            "Esta consulta involucra conceptos matemáticos. "
