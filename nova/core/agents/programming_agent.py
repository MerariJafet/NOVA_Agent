...
class ProgrammingAgent(BaseAgent):
    ...
    def __init__(self):
        ...
        self.programming_keywords = {
            'código', 'programar', 'programación', 'code', 'coding', 'programming',
            'función', 'clase', 'objeto', 'variable', 'algoritmo', 'algorithm',
            'debug', 'error', 'bug', 'exception', 'debugging', 'fix',
            'python', 'javascript', 'java', 'c++', 'php', 'ruby', 'go', 'rust',
            'html', 'css', 'sql', 'database', 'api', 'framework', 'library',
            'git', 'github', 'version', 'control', 'merge', 'commit', 'branch',
            'test', 'testing', 'unit', 'integration', 'tdd', 'ci', 'cd',
    ...
    def can_handle(self, query: str, context: Optional[Dict] = None) -> float:
        ...
        code_indicators = ['```', 'def ', 'class ', 'function', 'import ', 'from ', 'print(']
        ...
        for pattern in self.high_confidence_patterns:
            if re.search(pattern, query_lower, re.IGNORECASE):
                ...
                logger.info("programming_high_confidence_pattern",
        ...
        languages = ['python', 'javascript', 'java', 'c\\+\\+', 'php', 'ruby', 'go', 'rust', 'typescript']
        language_count = sum(1 for lang in languages
        ...
        logger.info("programming_confidence_calculated",
                   agent_id=self.agent_id,
    ...
    async def process_query(
        ...
        **kwargs
        ...
        try:
            ...
            logger.info("programming_query_processed",
                       agent_id=self.agent_id,
            ...
            logger.error("programming_query_failed",
                        agent_id=self.agent_id,
                        error=str(e),
                        response_time=response_time)
    ...
    def _analyze_programming_query(self, query: str) -> Dict[str, Any]:
        ...
        # Detectar lenguajes
        languages = ['python', 'javascript', 'java', 'c++', 'php', 'ruby', 'go', 'rust', 'typescript', 'sql']
        for lang in languages:
            if re.search(r'\b' + lang + r'\b', query_lower):
                analysis['languages'].append(lang)

        # Detectar código
        if '```' in query or any(keyword in query_lower for keyword in ['def ', 'class ', 'function', 'import ']):
        ...
        if any(word in query_lower for word in ['error', 'bug', 'problema', 'no funciona']):
            analysis['query_type'] = 'debugging'
        elif any(word in query_lower for word in ['cómo', 'como', 'tutorial', 'aprender']):
        ...
        elif any(word in query_lower for word in ['diseño', 'arquitectura', 'estructura']):
        ...
        if analysis['query_type'] == 'debugging' and analysis['has_code']:
            ...
            analysis['difficulty'] = 'high'
        elif analysis['has_code'] or analysis['query_type'] in ['architecture', 'optimization']:
    ...
    def _analyze_code_snippets(self, query: str) -> Dict[str, Any]:
        ...
        for i, block in enumerate(code_blocks):
            ...
            if 'except:' in block and ':' not in block.split('except:')[1][:50]:  # Bare except
                issues.append('Uso de except vacío (muy amplio)')
            if len(block.split('\n')) > 50:  # Long function
                issues.append('Función muy larga (considerar dividir)')

            analysis['potential_issues'].extend([f'Snippet {i+1}: {issue}' for issue in issues])
        ...
        has_functions = any('def ' in block or 'function' in block for block in code_blocks)
    ...
    def _generate_programming_recommendations(self, analysis: Dict[str, Any]) -> List[str]:
        ...
        if difficulty == 'high':
            recommendations.insert(0, '🔴 COMPLEJO: Divide el problema en partes más pequeñas')
        elif difficulty == 'low':
