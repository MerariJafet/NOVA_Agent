...
class AgentRegistry:
    ...
    def register(self, agent: BaseAgent) -> bool:
        ...
        if not isinstance(agent, BaseAgent):
            logger.error("invalid_agent_type", agent_type=type(agent).__name__)
            return False
    ...
    def unregister(self, agent_id: str) -> bool:
        ...
        self._agents_by_specialty[specialty] = [
            a for a in self._agents_by_specialty[specialty] if a.agent_id != agent_id
        ]
    ...
    def find_capable_agents(
        ...
        for agent in self.get_enabled_agents():
            try:
                ...
                logger.error(
                    "agent_evaluation_failed", agent_id=agent.agent_id, error=str(e)
                )
    ...
    def get_best_agent(
        self, query: str, context: Optional[Dict] = None, min_confidence: float = 0.5
        ...
        """
        capable = self.find_capable_agents(query, context, min_confidence, max_agents=1)
