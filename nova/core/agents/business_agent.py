...
class BusinessAgent(BaseAgent):
    ...
    def can_handle(self, query: str, context: Optional[Dict] = None) -> float:
        ...
        for pattern in self.high_confidence_patterns:
            if re.search(pattern, query_lower, re.IGNORECASE):
                logger.info(
                    "business_high_confidence_pattern",
        ...
        keyword_count = sum(
            1 for keyword in self.business_keywords if keyword in query_lower
        )
        ...
        logger.info(
            "business_confidence_calculated",
            agent_id=self.agent_id,
    ...
    def _analyze_business_query(self, query: str) -> Dict[str, Any]:
        ...
        if any(
            word in query_lower for word in ["empezar", "montar", "crear", "startup"]
        ):
            ...
            word in query_lower
            for word in ["finanzas", "dinero", "ganancias", "costos"]
        ):
        ...
        # Determinar urgencia
        if any(
            word in query_lower for word in ["urgente", "inmediato", "rápido", "ya"]
        ):
    ...
    def _generate_recommendations(self, analysis: Dict[str, Any]) -> List[str]:
        ...
        if urgency == "high":
            recommendations.insert(
                0, "🚨 PRIORIDAD: Esta consulta requiere atención inmediata"
            )
        elif urgency == "low":
            recommendations.append(
                "📅 Esta es una decisión estratégica que puede tomarse con tiempo"
            )
    ...
    def _suggest_next_steps(self, analysis: Dict[str, Any]) -> List[str]:
        ...
        if complexity == "high":
            next_steps.insert(
                0, "0. Divide el problema en partes más pequeñas y manejables"
            )
        elif complexity == "low":
